...
* RA/Dec: target coordinates.  These fields will reflect the selected Target but also support manual entry.
...
* Days: set the number of days from Start Date in the report.
* Minimum Altitude: set the minimum altitude to drive the start/end imaging times.  Use 'Above Horizon' to use your custom local horizon.
...
* When using a custom local horizon, be aware that the tool will use the target/horizon crossing closest to the set or rise event for the target (altitude = 0).  For example, if you have a tree in the southwest but then a lower horizon to the west of that, your target may first disappear behind the tree but then appear later.  The NINA 'Wait Until Above Horizon' instruction would stop you just east of the tree while the plugin wouldn't stop the session until the horizon closest to target setting.
